async def func_orchestrator_obj_create(*, user_id: any, api_role: str, obj_query: dict, obj_body: dict, config_table_create_my: list, config_table_create_public: list, config_column_blocked: list, config_table: dict, config_channel_allowed: list, client_celery_producer: any, client_kafka_producer: any, client_rabbitmq_producer: any, client_redis_producer: any, func_orchestrator_producer: callable, func_postgres_create: callable, client_postgres_pool: any, func_postgres_serialize: callable, cache_postgres_schema: dict, cache_postgres_buffer: dict) -> any:
    ...
    if obj_query.get("queue"):
        ...
        for i in range(0, len(obj_list), limit_batch):
            ...
            task_obj = {"task_name": "func_postgres_create", "params": {"mode": mode, "table": table, "obj_list": batch, "is_serialize": is_serialize, "buffer_limit": buffer_limit}}
            res = await func_orchestrator_producer(queue=obj_query.get("queue"), task_obj=task_obj, config_channel_allowed=config_channel_allowed, client_celery_producer=client_celery_producer, client_kafka_producer=client_kafka_producer, client_rabbitmq_producer=client_rabbitmq_producer, client_redis_producer=client_redis_producer)
            results.append(res)
...
async def func_orchestrator_obj_update(*, user_id: any, api_role: str, obj_query: dict, obj_body: dict, config_is_otp_users_update_admin: int, config_column_blocked: list, config_column_single_update: list, func_otp_verify: callable, client_postgres_pool: any, config_expiry_sec_otp: int, config_channel_allowed: list, client_celery_producer: any, client_kafka_producer: any, client_rabbitmq_producer: any, client_redis_producer: any, func_orchestrator_producer: callable, func_postgres_update: callable, func_postgres_serialize: callable, cache_postgres_schema: dict) -> any:
    ...
    if obj_query.get("queue"):
        ...
        for i in range(0, len(obj_list), limit_batch):
            batch = obj_list[i : i + limit_batch]
            task_obj = {"task_name": "func_postgres_update", "params": {"table": obj_query.get("table"), "obj_list": batch, "is_serialize": is_serialize, "created_by_id": created_by_id, "is_return_ids": is_return_ids}}
            res = await func_orchestrator_producer(queue=obj_query.get("queue"), task_obj=task_obj, config_channel_allowed=config_channel_allowed, client_celery_producer=client_celery_producer, client_kafka_producer=client_kafka_producer, client_rabbitmq_producer=client_rabbitmq_producer, client_redis_producer=client_redis_producer)
            results.append(res)
        return results if len(results) > 1 else results[0]

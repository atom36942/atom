...
async def middleware(request, api_function):
    ...
    try:
        ...
        error, response = await request.app.state.func_api_response_error(exception=e, is_traceback=request.app.state.config_is_enable_traceback, sentry_dsn=request.app.state.config_sentry_dsn)
    await request.app.state.func_api_log_create(config_is_enable_log_api=request.app.state.config_is_enable_log_api, api_id=request.app.state.config_api.get(request.url.path, {}).get("id"), request=request, response=response, time_ms=int((time.perf_counter() - start) * 1000), user_id=request.state.user.get("id") if getattr(request.state, "user", None) else None, description=error, func_postgres_create=request.app.state.func_postgres_create, client_postgres_pool=request.app.state.client_postgres_pool, client_password_hasher=request.app.state.client_password_hasher, func_postgres_serialize=request.app.state.func_postgres_serialize, cache_postgres_schema=request.app.state.cache_postgres_schema, cache_postgres_buffer=request.app.state.cache_postgres_buffer, config_table=request.app.state.config_table)
    return response

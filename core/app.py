...
async def middleware(request, api_function):
    try:
        ...
        request.state.user = await st.func_authenticate(header_dict=request.headers, route_path=request.url.path, config_token_secret_key=st.config_token_secret_key, config_api_roles_auth=st.config_api_roles_auth)
        await st.func_check_admin(user=request.state.user, route_path=request.url.path, config_api=st.config_api, client_postgres_pool=st.client_postgres_pool, client_redis=st.client_redis, cache_users_role=st.cache_users_role, config_redis_cache_ttl_sec=st.config_redis_cache_ttl_sec)
        await st.func_check_is_active(user=request.state.user, route_path=request.url.path, config_api=st.config_api, client_postgres_pool=st.client_postgres_pool, client_redis=st.client_redis, cache_users_is_active=st.cache_users_is_active, config_redis_cache_ttl_sec=st.config_redis_cache_ttl_sec)
        await st.func_check_ratelimiter(client_redis_ratelimiter=st.client_redis_ratelimiter, config_api=st.config_api, route_path=request.url.path, identifier_key=request.state.user.get("id") if request.state.user else request.client.host, cache_ratelimiter=st.cache_ratelimiter)
        response, type = await st.func_api_response(request_obj=request, api_function=api_function, config_api=st.config_api, client_redis=st.client_redis, identifier_val=request.state.user.get("id") if request.state.user else 0, func_api_response_background=st.func_api_response_background, func_check_cache=st.func_check_cache, cache_api_response=st.cache_api_response)
    except Exception as e:
        error, response = await request.app.state.func_api_response_error(error_obj=e, config_is_traceback=request.app.state.config_is_traceback, config_sentry_dsn=request.app.state.config_sentry_dsn)
    await request.app.state.func_api_log_create(config_is_log_api=request.app.state.config_is_log_api, api_id=request.app.state.config_api.get(request.url.path, {}).get("id"), request_obj=request, response_obj=response, time_ms=int((time.perf_counter() - start) * 1000), user_id=request.state.user.get("id") if getattr(request.state, "user", None) else None, func_postgres_object_create=request.app.state.func_postgres_object_create, client_postgres_pool=request.app.state.client_postgres_pool, func_postgres_serialize=request.app.state.func_postgres_serialize, cache_postgres_schema=request.app.state.cache_postgres_schema, cache_postgres_buffer=request.app.state.cache_postgres_buffer, config_table=request.app.state.config_table)
    return response

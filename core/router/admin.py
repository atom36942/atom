    ...
    async with app_state.client_postgres_pool.acquire() as conn:
        async with conn.transaction():
            async for ol in app_state.func_api_file_to_chunks(upload_file=of["file"][-1], chunk_size=5000):
                ...
                if of["mode"] == "create":
                    await app_state.func_postgres_create(client_postgres_pool=app_state.client_postgres_pool, client_postgres_conn=conn, client_password_hasher=app_state.client_password_hasher, func_postgres_serialize=app_state.func_postgres_serialize, cache_postgres_schema=app_state.cache_postgres_schema, mode="now", table=of["table"], obj_list=ol, is_serialize=of["is_serialize"], buffer_limit=0, cache_postgres_buffer=app_state.cache_postgres_buffer, config_regex=app_state.config_regex, func_regex_check=app_state.func_regex_check, config_obj_list_limit=app_state.config_obj_list_limit)
                elif of["mode"] == "update":

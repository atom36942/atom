...
async def func_api_admin_sync(*, request: Request):
    ...
    app_state.cache_users_deactivated = await app_state.func_postgres_map_column(client_postgres_pool=app_state.client_postgres_pool, config_sql=app_state.config_sql.get("users_is_active")) if app_state.client_postgres_pool else {}
...
async def func_api_admin_object_create(*, request: Request):
    ...
    if request.state.user.get("id") and "created_by_id" in app_state.cache_postgres_schema.get(oq["table"], {}): obj_list = [dict(item, created_by_id=request.state.user["id"]) for item in obj_list]
...
async def func_api_admin_object_update(*, request: Request):
    ...
    if request.state.user.get("id") and "updated_by_id" in app_state.cache_postgres_schema.get(oq["table"], {}): obj_list = [dict(item, updated_by_id=request.state.user["id"]) for item in obj_list]
    created_by_id = None

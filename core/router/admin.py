...
async def func_api_admin_postgres_runner(*, request: Request):
    app_state = request.app.state
    ob = await app_state.func_request_param_read(request=request, mode="body", strict=0, config=[("mode", "str", 1, ["read", "write"], None), ("query", "str", 1, None, None)])
    output = await app_state.func_postgres_runner(client_postgres_pool=app_state.client_postgres_pool, mode=ob["mode"], query=ob["query"])

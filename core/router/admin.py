...
async def func_api_admin_postgres_import(*, request: Request):
    ...
    count = await app_state.func_orchestrator_semaphore_postgres_import(upload_file=obj_f["file"][-1], mode=obj_f["mode"], table=obj_f["table"], is_serialize=obj_f["is_serialize"], client_postgres_pool=app_state.client_postgres_pool, client_password_hasher=app_state.client_password_hasher, cache_postgres_schema=app_state.cache_postgres_schema, cache_postgres_buffer=app_state.cache_postgres_buffer, func_postgres_serialize=app_state.func_postgres_serialize, func_postgres_create=app_state.func_postgres_create, func_postgres_update=app_state.func_postgres_update, func_postgres_delete=app_state.func_postgres_delete, func_api_file_to_chunks=app_state.func_api_file_to_chunks)

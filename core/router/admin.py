...
async def func_api_admin_postgres_runner(*, request: Request):
    ...
    ql = ob["query"].lower().strip()
    if any(re.search(rf"\b{k}\b", ql) for k in ("drop", "truncate", "delete")): raise Exception("forbidden keyword in query")
    ...
    async with app_state.client_postgres_pool.acquire() as conn:
        if ob["mode"] == "read" or ql.startswith(("select", "with", "explain", "show", "describe")) or "returning" in ql:
            return {"status": 1, "message": [dict(r) for r in await conn.fetch(ob["query"], timeout=15)]}
        return {"status": 1, "message": await conn.execute(ob["query"], timeout=15)}
...
async def func_api_admin_postgres_export(*, request: Request):
    ...
    import io; from fastapi.responses import StreamingResponse
    async def _iter():
        async with app_state.client_postgres_pool.acquire() as conn:
            output = io.BytesIO()
            await conn.copy_from_query(f"SELECT * FROM {oq['table']}", output=output, format='csv', header=True)
            yield output.getvalue()
    return StreamingResponse(_iter(), media_type="text/csv", headers={"Content-Disposition": f"attachment; filename={oq['table']}.csv"})
...
async def func_api_admin_postgres_import(*, request: Request):
    ...
    async for ol in app_state.func_api_file_to_chunks(upload_file=of["file"][-1], chunk_size=limit_batch):
        if of["mode"] == "create": await app_state.func_postgres_create(client_postgres_pool=app_state.client_postgres_pool, client_password_hasher=app_state.client_password_hasher, func_postgres_serialize=app_state.func_postgres_serialize, cache_postgres_schema=app_state.cache_postgres_schema, mode="now", table=of["table"], obj_list=ol, is_serialize=of["is_serialize"], buffer_limit=0, cache_postgres_buffer=app_state.cache_postgres_buffer, client_postgres_conn=None)
        elif of["mode"] == "update": await app_state.func_postgres_update(client_postgres_pool=app_state.client_postgres_pool, client_password_hasher=app_state.client_password_hasher, func_postgres_serialize=app_state.func_postgres_serialize, cache_postgres_schema=app_state.cache_postgres_schema, mode="now", table=of["table"], obj_list=ol, is_serialize=of["is_serialize"], created_by_id=None, is_return_ids=0, buffer_limit=0, cache_postgres_buffer=app_state.cache_postgres_buffer, client_postgres_conn=None)
        elif of["mode"] == "delete":
            ids = ",".join([str(item["id"]) for item in ol if "id" in item])
            if ids: await app_state.func_postgres_delete(client_postgres_pool=app_state.client_postgres_pool, table=of["table"], ids=ids, created_by_id=None, client_postgres_conn=None)
        count += len(ol)
...
async def func_api_admin_mongodb_import(*, request: Request):
    ...
    if of["mode"] == "create":
        ...
        async for ol in app_state.func_api_file_to_chunks(upload_file=of["file"][-1], chunk_size=limit_batch):
            for item in ol:
                if (oid := item.pop("id", None)) or (oid := item.pop("_id", None)):
                    await app_state.client_mongodb[of["database"]][of["table"]].update_one({"_id": oid}, {"$set": item})
            count += len(ol)
    elif of["mode"] == "delete":
        async for ol in app_state.func_api_file_to_chunks(upload_file=of["file"][-1], chunk_size=limit_batch):
            ids = [item.get("id") or item.get("_id") for item in ol if item.get("id") or item.get("_id")]
            if ids: await app_state.client_mongodb[of["database"]][of["table"]].delete_many({"_id": {"$in": ids}})
            count += len(ol)
    return {"status": 1, "message": f"{count} rows processed"}
...
async def func_api_admin_blob_url_delete(*, request: Request):
    ...
    service, urls = oq["service"], ob["url"]
    tasks = []
    if service == "s3":
        batches = {}
        for u in urls:

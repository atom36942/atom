...
async def func_api_private_blob_upload_file(request:Request):
   ...
   if of["service"] == "s3":
      ...
      output=await app_state.func_azure_upload_file(client_azure_blob=app_state.client_azure_blob, container=container, file_list=of["file"], config_blob_limit_kb=app_state.config_blob_limit_kb, config_blob_upload_limit_count=app_state.config_blob_upload_limit_count)
...
async def func_api_private_blob_upload_url(request:Request):
   ...
   if oq["service"] == "s3":
      output=app_state.func_s3_upload_url(client_s3=app_state.client_s3, config_s3_region_name=app_state.config_s3_region_name, bucket=container, config_blob_limit_kb=app_state.config_blob_limit_kb, config_blob_expire_sec=app_state.config_blob_expire_sec, count=oq["count"], config_blob_upload_limit_count=app_state.config_blob_upload_limit_count)
   elif oq["service"] == "azure":
      output=app_state.func_azure_upload_url(client_azure_blob=app_state.client_azure_blob, config_azure_account_name=app_state.config_azure_account_name, config_azure_account_key=app_state.config_azure_account_key, container=container, config_blob_limit_kb=app_state.config_blob_limit_kb, config_blob_expire_sec=app_state.config_blob_expire_sec, count=oq["count"], config_blob_upload_limit_count=app_state.config_blob_upload_limit_count)
   return {"status":1,"message":output}

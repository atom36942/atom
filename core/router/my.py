...
async def func_api_my_api_usage(*, request: Request):
    ...
    obj_list = await app_state.func_user_api_usage_read(client_postgres_pool=app_state.client_postgres_pool, days=oq["days"], user_id=request.state.user["id"])
...
async def func_api_my_account_delete(*, request: Request):
    ...
    output = await app_state.func_user_account_delete(mode=oq["mode"], client_postgres_pool=app_state.client_postgres_pool, user_id=request.state.user["id"])
...
async def func_api_my_message_received(*, request: Request):
    ...
    obj_list = await app_state.func_message_received(client_postgres_pool=app_state.client_postgres_pool, user_id=request.state.user["id"], mode=oq["mode"], order=oq["order"], limit=oq["limit"], page=oq["page"])
    return {"status": 1, "message": obj_list}
...
async def func_api_my_message_inbox(*, request: Request):
    app_state = request.app.state
    oq = await app_state.func_request_param_read(request=request, mode="query", strict=0, config=[("mode", "str", 1, ["all", "unread", "read"], None), ("order", "str", 0, None, "id desc"), ("limit", "int", 0, None, 100), ("page", "int", 0, None, 1)])
    obj_list = await app_state.func_message_inbox(client_postgres_pool=app_state.client_postgres_pool, user_id=request.state.user["id"], mode=oq["mode"], order=oq["order"], limit=oq["limit"], page=oq["page"])
    return {"status": 1, "message": obj_list}
...
async def func_api_my_message_thread(*, request: Request):
    ...
    obj_list = await app_state.func_message_thread(client_postgres_pool=app_state.client_postgres_pool, user_one_id=request.state.user["id"], user_id=oq["user_id"], order=oq["order"], limit=oq["limit"], page=oq["page"])
    asyncio.create_task(app_state.func_message_thread_mark_read(client_postgres_pool=app_state.client_postgres_pool, current_user_id=request.state.user["id"], partner_id=oq["user_id"]))
    return {"status": 1, "message": obj_list}
...
async def func_api_my_message_delete_single(*, request: Request):
    ...
    output = await app_state.func_message_delete_single(client_postgres_pool=app_state.client_postgres_pool, id=oq["id"], user_id=request.state.user["id"])
    return {"status": 1, "message": output}
...
async def func_api_my_message_delete_bulk(*, request: Request):
    ...
    output = await app_state.func_message_delete_bulk(client_postgres_pool=app_state.client_postgres_pool, user_id=request.state.user["id"], mode=oq["mode"])
    return {"status": 1, "message": output}
...
async def func_api_my_ids_delete(*, request: Request):
    ...
    output = await app_state.func_postgres_delete(client_postgres_pool=app_state.client_postgres_pool, table=ob["table"], ids=ob["ids"], created_by_id=request.state.user.get("id", 0))
    return {"status": 1, "message": output}
...
async def func_api_my_object_create(*, request: Request):
    ...
    return {"status": 1, "message": await app_state.func_orchestrator_obj_create(user_id=request.state.user["id"], api_role="my", table=oq["table"], mode=oq["mode"], is_serialize=oq["is_serialize"], queue=oq["queue"], obj_list=obj_list, config_table_create_my=app_state.config_table_create_my, config_table_create_public=app_state.config_table_create_public, config_column_blocked=app_state.config_column_blocked, config_table=app_state.config_table, config_regex=app_state.config_regex, func_regex_check=app_state.func_regex_check, client_celery_producer=app_state.client_celery_producer, client_kafka_producer=app_state.client_kafka_producer, client_rabbitmq_producer=app_state.client_rabbitmq_producer, client_redis_producer=app_state.client_redis_producer, func_orchestrator_producer=app_state.func_orchestrator_producer, func_postgres_create=app_state.func_postgres_create, client_postgres_pool=app_state.client_postgres_pool, client_password_hasher=app_state.client_password_hasher, func_postgres_serialize=app_state.func_postgres_serialize, cache_postgres_schema=app_state.cache_postgres_schema, cache_postgres_buffer=app_state.cache_postgres_buffer)}

@router.put("/my/object-update")
async def func_api_my_object_update(*, request: Request):
    app_state = request.app.state
    oq = await app_state.func_request_param_read(request=request, mode="query", strict=0, config=[("table", "str", 1, app_state.cache_postgres_schema_tables, None), ("is_serialize", "int", 0, [0, 1], 0), ("otp", "int", 0, None, None), ("queue", "str", 0, None, None)])
    ob = await app_state.func_request_param_read(request=request, mode="body", strict=0, config=[])
    obj_list = app_state.func_request_obj_list_read(obj_body=ob)
    return {"status": 1, "message": await app_state.func_orchestrator_obj_update(user_id=request.state.user["id"], api_role="my", table=oq["table"], is_serialize=oq["is_serialize"], queue=oq["queue"], otp=oq["otp"], obj_list=obj_list, config_is_otp_users_update_admin=app_state.config_is_otp_users_update_admin, config_column_blocked=app_state.config_column_blocked, config_column_single_update=app_state.config_column_single_update, config_regex=app_state.config_regex, func_regex_check=app_state.func_regex_check, func_otp_verify=app_state.func_otp_verify, client_postgres_pool=app_state.client_postgres_pool, client_password_hasher=app_state.client_password_hasher, config_expiry_sec_otp=app_state.config_expiry_sec_otp, client_celery_producer=app_state.client_celery_producer, client_kafka_producer=app_state.client_kafka_producer, client_rabbitmq_producer=app_state.client_rabbitmq_producer, client_redis_producer=app_state.client_redis_producer, func_orchestrator_producer=app_state.func_orchestrator_producer, func_postgres_update=app_state.func_postgres_update, func_postgres_serialize=app_state.func_postgres_serialize, cache_postgres_schema=app_state.cache_postgres_schema)}

...
async def func_api_my_object_update(*, request: Request):
    ...
    ob = await app_state.func_request_param_read(request=request, mode="body", strict=0, config=[])
    obj_list = app_state.func_request_obj_list_read(obj_body=ob)
    return {"status": 1, "message": await app_state.func_orchestrator_obj_update(user_id=request.state.user["id"], api_role="my", table=oq["table"], is_serialize=oq["is_serialize"], queue=oq["queue"], otp=oq["otp"], obj_list=obj_list, config_is_enable_otp_users_update_admin=app_state.config_is_enable_otp_users_update_admin, config_column_blocked=app_state.config_column_blocked, config_column_single_update=app_state.config_column_single_update, config_regex=app_state.config_regex, func_regex_check=app_state.func_regex_check, func_otp_verify=app_state.func_otp_verify, client_postgres_pool=app_state.client_postgres_pool, client_password_hasher=app_state.client_password_hasher, config_expiry_sec_otp=app_state.config_expiry_sec_otp, client_celery_producer=app_state.client_celery_producer, client_kafka_producer=app_state.client_kafka_producer, client_rabbitmq_producer=app_state.client_rabbitmq_producer, client_redis_producer=app_state.client_redis_producer, func_orchestrator_producer=app_state.func_orchestrator_producer, func_postgres_update=app_state.func_postgres_update, func_postgres_serialize=app_state.func_postgres_serialize, cache_postgres_schema=app_state.cache_postgres_schema, client_postgres_conn=None)}

...
# Base columns to ignore during schema generation and AI parsing
BASE_COLUMNS = {
...
async def func_claim_candidates(conn: asyncpg.Connection, batch_limit: int) -> list:
    ...
    return await conn.fetch(
        ...
        WITH claim AS (
            ...
            FROM candidate c
            WHERE (
                ((c.worker_status IN (1, 4) OR c.worker_status IS NULL) AND (c.worker_next_retry_at <= NOW() OR c.worker_next_retry_at IS NULL))
                OR (c.worker_status = 2 AND c.updated_at < NOW() - INTERVAL '15 minutes')
              )
        ...
        UPDATE candidate u
        SET worker_status = 2, -- 2: Processing
            updated_at = NOW()
        FROM claim
...
async def func_mark_completed(conn: asyncpg.Connection, candidate_id: int, data: dict) -> None:
    # Filter the data to only include valid columns from the candidate table
    ...
    if not update_data:
        # If nothing was extracted, just mark as completed
        await conn.execute(
            "UPDATE candidate SET worker_status = 3, worker_processed_at = NOW(), worker_last_error = NULL, updated_at = NOW() WHERE id = $1",
            candidate_id
        )
        return

    # Build dynamic UPDATE query securely
...
def func_retry_delay_sec(retry_count: int) -> int:
    delays = [60, 300, 3600, 86400] # 1m, 5m, 1h, 1d
...
async def func_mark_failed(conn: asyncpg.Connection, candidate_id: int, retry_count: int, error: Exception) -> None:
    ...
    await conn.execute(
        """
        UPDATE candidate
        SET worker_status = 4, -- 4: Failed
            worker_last_error = $2,
            worker_retry_count = worker_retry_count + 1,
            worker_next_retry_at = NOW() + ($3 * INTERVAL '1 second'),
            updated_at = NOW()
        WHERE id = $1
        """,
        candidate_id,
        f"AI Parsing Failed: {error_str}",
        delay_sec
    )

async def func_process_candidate(candidate: asyncpg.Record, client_gemini: genai.Client) -> dict:
    ...
    # Smart Signing Logic for AWS S3
    ...
    try:
        ...
        # Upload & Extract via Gemini
        def process_with_gemini():
            uploaded_file = client_gemini.files.upload(file=temp_file_path)
            
            # Dynamic schema based on config_postgres
...
async def process_and_update(pool: asyncpg.Pool, client_gemini: genai.Client, candidate: asyncpg.Record):
    try:
        data = await func_process_candidate(candidate, client_gemini)
...
async def func_resume_parser_worker_once(pool: asyncpg.Pool, client_gemini: genai.Client) -> int:
    async with pool.acquire() as conn:
        async with conn.transaction():
            # Lock and claim candidates
            candidates = await func_claim_candidates(conn, 5)
            
    if candidates:
        print(f"[resume-parser-worker] claimed {len(candidates)} candidate(s)")
        # Process candidates sequentially to respect Gemini API Rate Limits (max 15 requests per minute on free tier)
        for candidate in candidates:
            await process_and_update(pool, client_gemini, candidate)
            await asyncio.sleep(0.1) # Yield to event loop
            
    return len(candidates)

async def func_resume_parser_worker():
    ...
    try:
        idle_count = 0
        while True:
            processed = await func_resume_parser_worker_once(pool, client_gemini)
            if processed:
                idle_count = 0

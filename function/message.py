...
async def func_message_received(client_postgres_pool: any, user_id: int, mode: str = None, sort_order: str = None, limit_count: int = None, page_number: int = None, func_postgres_ids_update: callable = None, func_validate_identifier: callable = None) -> list:
    ...
    async with client_postgres_pool.acquire() as conn:
        ...
        if obj_list and func_postgres_ids_update:
            ...
            if mark_read_ids:
                asyncio.create_task(func_postgres_ids_update(client_postgres_pool, "message", mark_read_ids, "is_read", 1, func_validate_identifier))
    return obj_list

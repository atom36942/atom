...
async def function_api_root_postgres_import(request:Request):
   ...
   if param["mode"]=="create":output=await function_object_create_postgres(request.app.state.client_postgres_pool,param["table"],object_list,1,function_object_serialize,request.app.state.cache_postgres_column_datatype)

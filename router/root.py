...
async def function_api_root_postgres_import(request:Request):
   ...
   elif param["mode"]=="update":output=await function_postgres_object_update(request.app.state.client_postgres_pool,param["table"],object_list)
   elif param["mode"]=="delete":output=await function_object_delete_postgres(request.app.state.client_postgres_pool,param["table"],object_list,1,request.app.state.cache_postgres_column_datatype)
   return {"status":1,"message":output}

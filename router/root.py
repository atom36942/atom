...
async def function_api_root_postgres_query_runner(request:Request):
    ...
    elif obj["queue"]=="celery": output=await function_celery_producer(request.app.state.client_celery_producer,"function_postgres_query_runner",[param["query"]])
    elif obj["queue"]=="kafka": output=await function_kafka_producer(request.app.state.client_kafka_producer,"channel_1",{"function":"function_postgres_query_runner","query":param["query"]})
    elif obj["queue"]=="rabbitmq": output=await function_rabbitmq_producer(request.app.state.client_rabbitmq_producer,"channel_1",{"function":"function_postgres_query_runner","query":param["query"]})
    elif obj["queue"]=="redis": output=await function_redis_producer(request.app.state.client_redis_producer,"channel_1",{"function":"function_postgres_query_runner","query":param["query"]})
    return {"status":1,"message":output}

@router.post("/root/postgres-export")
async def function_api_root_postgres_export(request:Request):
   param=await function_param_read(request,"body",[["query",None,1,None]])
   stream=function_postgres_query_read_stream(request.app.state.client_postgres,param["query"])
...
async def function_api_root_postgres_import(request:Request):
   ...
   if param["mode"]=="create":output=await function_object_create_postgres(request.app.state.client_postgres,param["table"],object_list,1,function_object_serialize,request.app.state.cache_postgres_column_datatype)
   elif param["mode"]=="update":output=await function_object_update_postgres(request.app.state.client_postgres,param["table"],object_list,1,function_object_serialize,request.app.state.cache_postgres_column_datatype)
   elif param["mode"]=="delete":output=await function_object_delete_postgres(request.app.state.client_postgres,param["table"],object_list,1,function_object_serialize,request.app.state.cache_postgres_column_datatype)
   return {"status":1,"message":output}
...
async def function_api_root_redis_import_create(request:Request):
   ...
   await function_object_create_redis(request.app.state.client_redis,key_list,object_list,param["expiry_sec"])
   return {"status":1,"message":"done"}
...
async def function_api_root_redis_import_delete(request:Request):
   ...
   object_list=await function_file_to_object_list(param["file"][-1])
   await function_object_delete_redis(request.app.state.client_redis,object_list)
   return {"status":1,"message":"done"}
...
async def function_api_root_mongodb_import_create(request:Request):
   ...
   object_list=await function_file_to_object_list(param["file"][-1])
   output=await function_object_create_mongodb(request.app.state.client_mongodb,param["database"],param["table"],object_list)
   return {"status":1,"message":output}

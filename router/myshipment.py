...
@router.get("/myshipment/user-controlling-360")
async def func_api_myshipment_user_controlling_360(*, request: Request):
    app_state = request.app.state
    if not app_state.client_mssql_read_fallback: raise Exception("MSSQL client not initialized")
    oq = await app_state.func_request_param_read(request=request, mode="query", strict=0, config=[("limit", "int", 0, None, app_state.config_sql_read_limit_default), ("page", "int", 0, None, 1), ("search", "str", 0, None, "")])
    limit = max(1, int(oq["limit"] or app_state.config_sql_read_limit_default))
    if app_state.config_sql_read_limit_max and limit > app_state.config_sql_read_limit_max: raise Exception(f"query limit {limit} exceeds maximum allowed: {app_state.config_sql_read_limit_max}")
    page = max(1, int(oq["page"] or 1))
    offset = (page - 1) * limit
    sql_limit = limit + 1
    ...
    sql = f"""
        ...
        WITH ShipmentAgg AS (
            SELECT JSO.ControllingCustomer_PK AS org_pk,
                   COUNT(DISTINCT JS.JS_PK) AS total_shipments,
                   SUM(CASE WHEN JS.JS_IsBooking = 1 THEN 1 ELSE 0 END) AS total_bookings,
                   MAX(JS.JS_SystemCreateTimeUtc) AS last_activity_date
            FROM dbo.JobShipment JS
            INNER JOIN dbo.cvw_JobShipmentOrgs JSO ON JS.JS_PK = JSO.JS_PK
            WHERE JS.JS_IsValid = 1 AND JSO.ControllingCustomer_PK IS NOT NULL
            GROUP BY JSO.ControllingCustomer_PK
        )
        SELECT
            CONVERT(varchar(36), OH.OH_PK) AS org_id,
            OH.OH_Code AS org_code,
            ...
            SA.total_shipments AS total_shipments,
            SA.total_bookings AS total_bookings,
            CASE WHEN SA.last_activity_date IS NULL THEN NULL ELSE CONVERT(varchar(33), SA.last_activity_date, 126) + 'Z' END AS last_activity_date
        FROM ShipmentAgg SA
        INNER JOIN dbo.OrgHeader OH ON OH.OH_PK = SA.org_pk
        OUTER APPLY (
            SELECT TOP 1 OA.OA_RN_NKCountryCode AS CountryCode, OA.OA_City AS City
            FROM dbo.OrgAddress OA
    ...
    async with app_state.client_mssql_read_fallback.acquire() as conn:
        cursor = await conn.cursor()
        await cursor.execute(sql, search)
        columns = [column[0] for column in cursor.description]
        obj_list = [dict(zip(columns, row)) for row in await cursor.fetchall()]
    return {"status": 1, "message": {"obj_list": obj_list[:limit], "has_next_page": len(obj_list) > limit}}

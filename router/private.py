...
async def func_api_private_blob_upload_file(request:Request):
    ...
    if of["service"] == "s3":
        for item in of["file"]:
            ...
            output[item.filename] = file_url; blob_list.append({"created_by_id": request.state.user["id"], "upload_method": "upload_file", "service": of["service"], "container": container, "blob_key": file_key, "file_url": file_url})
        ...
        for item in of["file"]:
            ...
            output[item.filename] = blob_client.url; blob_list.append({"created_by_id": request.state.user["id"], "upload_method": "upload_file", "service": of["service"], "container": container, "blob_key": file_key, "file_url": blob_client.url})
...
async def func_api_private_blob_upload_url(request:Request):
    ...
    if oq["service"] == "s3":
        for _ in range(oq["count"]):
            ...
            output.append({"upload_url": presigned_post["url"], **presigned_post["fields"], "file_url": file_url}); blob_list.append({"created_by_id": request.state.user["id"], "upload_method": "upload_url", "service": oq["service"], "container": container, "blob_key": file_key, "file_url": file_url})
        ...
        for _ in range(oq["count"]):
            ...
            output.append({"upload_url": sas_url, "key": file_key, "file_url": file_url}); blob_list.append({"created_by_id": request.state.user["id"], "upload_method": "upload_url", "service": oq["service"], "container": container, "blob_key": file_key, "file_url": file_url})
    if blob_list: await app_state.func_postgres_create(client_postgres_pool=app_state.client_postgres_pool, client_postgres_conn=None, client_password_hasher=app_state.client_password_hasher, func_postgres_serialize=app_state.func_postgres_serialize, func_regex_check=app_state.func_regex_check, cache_postgres_schema=app_state.cache_postgres_schema, cache_postgres_buffer_create=app_state.cache_postgres_buffer_create, config_regex=app_state.config_regex, buffer_limit=app_state.config_buffer_limit_default, mode="now", table="blob", obj_list=blob_list)
    return {"status":1,"message":output}
...
async def func_api_private_blob_preview_urls(request:Request):
    ...
    if of["service"] == "s3":
        for file_url in of["urls"]:
            ...
            url = app_state.client_s3.generate_presigned_url(ClientMethod='get_object', Params={'Bucket': dynamic_container, 'Key': file_key}, ExpiresIn=app_state.config_blob_expire_sec_preview)
            output[file_url] = url
    elif of["service"] == "azure":
        for file_url in of["urls"]:
            dynamic_container, file_key = file_url.split(".net/")[1].split("?")[0].split("/", 1)
            sas_token = generate_blob_sas(account_name=app_state.config_azure_account_name, account_key=app_state.config_azure_account_key, container_name=dynamic_container, blob_name=file_key, permission=BlobSasPermissions(read=True), expiry=datetime.now(timezone.utc) + timedelta(seconds=app_state.config_blob_expire_sec_preview))

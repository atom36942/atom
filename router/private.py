...
async def function_api_private_s3_upload_file(request:Request):
   ...
   for index,f in enumerate(param["file"]):
      ...
      output[f.filename]=await function_s3_upload_file(request.app.state.client_s3,config_s3_region_name,param["bucket"],f,k,config_limit_s3_kb)
   return {"status":1,"message":output}

@router.get("/private/s3-upload-presigned")
async def function_api_private_s3_upload_presigned(request:Request):
   param=await function_request_param_read(request,"query",[["bucket","str",1,None],["key","str",0,None]])
   output=function_s3_upload_presigned(request.app.state.client_s3,config_s3_region_name,param["bucket"],param["key"],config_limit_s3_kb,config_s3_presigned_expire_sec)
   return {"status":1,"message":output}

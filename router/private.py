...
async def function_api_private_s3_upload_file(request:Request):
   ...
   for index,f in enumerate(param["file"]):
      ...
      output[f.filename]=await function_s3_upload_file(param["bucket"],key,f,request.app.state.client_s3,config_s3_region_name,config_limit_s3_kb)
...
async def function_api_private_s3_upload_presigned(request:Request):
   ...
   output=function_s3_upload_presigned(param["bucket"],param["key"],request.app.state.client_s3,config_s3_region_name,config_limit_s3_kb,config_s3_presigned_expire_sec)
   return {"status":1,"message":output}

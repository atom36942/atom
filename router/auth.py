...
async def func_api_auth_signup_username_password(*, request:Request):
    ...
    ob = await app_state.func_request_param_read(request=request, mode="body", strict=False, param_specs=[{"name": "role", "type": "int", "required": True, "allowed": app_state.config_allowed_users_role, "default": None}, {"name": "username", "type": "str", "required": True, "allowed": None, "default": None}, {"name": "password", "type": "str", "required": True, "allowed": None, "default": None}, {"name": "source", "type": "int", "required": False, "allowed": None, "default": None}])
    ...
    if not app_state.config_is_signup: raise Exception("signup disabled")
...
async def func_api_auth_login_username_password(*, request:Request):
    app_state = request.app.state
    if not app_state.client_postgres: raise Exception("postgres client not initialized")
    ob = await app_state.func_request_param_read(request=request, mode="body", strict=False, param_specs=[{"name": "role", "type": "int", "required": False, "allowed": app_state.config_allowed_users_role, "default": None}, {"name": "username", "type": "str", "required": True, "allowed": None, "default": None}, {"name": "password", "type": "str", "required": True, "allowed": None, "default": None}])
    if ob.get("username"): ob["username"] = ob["username"].strip()
    await app_state.func_regex_check(config_regex=app_state.config_regex, obj_list=[ob])
    async with app_state.client_postgres.acquire() as conn:
...
async def func_api_auth_login_email_password(*, request:Request):
    app_state = request.app.state
    if not app_state.client_postgres: raise Exception("postgres client not initialized")
    ob = await app_state.func_request_param_read(request=request, mode="body", strict=False, param_specs=[{"name": "role", "type": "int", "required": False, "allowed": app_state.config_allowed_users_role, "default": None}, {"name": "email", "type": "str", "required": True, "allowed": None, "default": None}, {"name": "password", "type": "str", "required": True, "allowed": None, "default": None}])
...
async def func_api_auth_login_mobile_password(*, request:Request):
    app_state = request.app.state
    if not app_state.client_postgres: raise Exception("postgres client not initialized")
    ob = await app_state.func_request_param_read(request=request, mode="body", strict=False, param_specs=[{"name": "role", "type": "int", "required": False, "allowed": app_state.config_allowed_users_role, "default": None}, {"name": "mobile", "type": "str", "required": True, "allowed": None, "default": None}, {"name": "password", "type": "str", "required": True, "allowed": None, "default": None}])
...
async def func_api_auth_login_google(*, request:Request):
    app_state = request.app.state
    if not app_state.client_postgres: raise Exception("postgres client not initialized")
    ob = await app_state.func_request_param_read(request=request, mode="body", strict=False, param_specs=[{"name": "role", "type": "int", "required": True, "allowed": app_state.config_allowed_users_role, "default": None}, {"name": "google_token", "type": "str", "required": True, "allowed": None, "default": None}, {"name": "source", "type": "int", "required": False, "allowed": None, "default": None}])

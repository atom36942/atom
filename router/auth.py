...
async def func_api_auth_signup_username_password_bigint(*, request:Request):
   st=request.app.state
   obj_body=await st.func_request_param_read(request=request, mode="body", config=[("type","int",1,None,None,None),("username_bigint","int",1,None,None,None),("password_bigint","int",1,None,None,"^[0-9]{6,12}$", "Password must be 6-12 digits")], strict=0)
...
async def func_api_auth_login_password_username(*, request:Request):
   st=request.app.state
   obj_body=await st.func_request_param_read(request=request, mode="body", config=[["type","int",1,None,None,None],["password","str",1,None,None,"^\\S{8,32}$", "Password must be 8-32 characters and contain no spaces"],["username","str",1,None,None,None]], strict=0)
   user=await st.func_auth_login_password_username(client_postgres_pool=st.client_postgres_pool, func_password_hash=st.func_password_hash, type=obj_body["type"], password=obj_body["password"], username=obj_body["username"])
   token=await st.func_token_encode(user=user, config_token_secret_key=st.config_token_secret_key, config_token_expiry_sec=st.config_token_expiry_sec, config_token_refresh_expiry_sec=st.config_token_refresh_expiry_sec, config_token_key=st.config_token_key)
   return {"status":1,"message":{"user":user,"token":token}}
...
async def func_api_auth_login_password_username_bigint(*, request:Request):
   st=request.app.state
   obj_body=await st.func_request_param_read(request=request, mode="body", config=[("type","int",1,None,None,None),("password_bigint","int",1,None,None,"^[0-9]{6,12}$", "Password must be 6-12 digits"),("username_bigint","int",1,None,None,None)], strict=0)
   user=await st.func_auth_login_password_username_bigint(client_postgres_pool=st.client_postgres_pool, type=obj_body["type"], password_bigint=obj_body["password_bigint"], username_bigint=obj_body["username_bigint"])
   token=await st.func_token_encode(user=user, config_token_secret_key=st.config_token_secret_key, config_token_expiry_sec=st.config_token_expiry_sec, config_token_refresh_expiry_sec=st.config_token_refresh_expiry_sec, config_token_key=st.config_token_key)
   return {"status":1,"message":{"user":user,"token":token}}
...
async def func_api_auth_login_password_email(*, request:Request):
   st=request.app.state
   obj_body=await st.func_request_param_read(request=request, mode="body", config=[("type","int",1,None,None,None),("password","str",1,None,None,"^\\S{8,32}$", "Password must be 8-32 characters and contain no spaces"),("email","str",1,None,None,None)], strict=0)
   user=await st.func_auth_login_password_email(client_postgres_pool=st.client_postgres_pool, func_password_hash=st.func_password_hash, type=obj_body["type"], password=obj_body["password"], email=obj_body["email"])
   token=await st.func_token_encode(user=user, config_token_secret_key=st.config_token_secret_key, config_token_expiry_sec=st.config_token_expiry_sec, config_token_refresh_expiry_sec=st.config_token_refresh_expiry_sec, config_token_key=st.config_token_key)
   return {"status":1,"message":{"user":user,"token":token}}
...
async def func_api_auth_login_password_mobile(*, request:Request):
   st=request.app.state
   obj_body=await st.func_request_param_read(request=request, mode="body", config=[("type","int",1,None,None,None),("password","str",1,None,None,"^\\S{8,32}$", "Password must be 8-32 characters and contain no spaces"),("mobile","str",1,None,None,None)], strict=0)
   user=await st.func_auth_login_password_mobile(client_postgres_pool=st.client_postgres_pool, func_password_hash=st.func_password_hash, type=obj_body["type"], password=obj_body["password"], mobile=obj_body["mobile"])
   token=await st.func_token_encode(user=user, config_token_secret_key=st.config_token_secret_key, config_token_expiry_sec=st.config_token_expiry_sec, config_token_refresh_expiry_sec=st.config_token_refresh_expiry_sec, config_token_key=st.config_token_key)
   return {"status":1,"message":{"user":user,"token":token}}
...
async def func_api_auth_login_google(*, request:Request):
   st=request.app.state
   obj_body=await st.func_request_param_read(request=request, mode="body", config=[("type","int",1,None,None,None,None),("google_token","str",1,None,None,None,None)], strict=0)

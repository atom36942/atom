...
async def function_api_auth_login_otp_email(request:Request):
    ...
    if param["type"] not in config_auth_type_list:raise Exception("type not allowed")
    token=await function_auth_login_otp_email(request.app.state.client_postgres_pool,param["type"],param["email"],function_otp_verify,param["otp"],function_token_encode,config_key_jwt,config_token_expire_sec,config_token_user_key_list)
    return {"status":1,"message":token}
...
async def function_api_auth_login_otp_mobile(request:Request):
    ...
    if param["type"] not in config_auth_type_list:raise Exception("type not allowed")
    token=await function_auth_login_otp_mobile(request.app.state.client_postgres_pool,param["type"],param["mobile"],function_otp_verify,param["otp"],function_token_encode,config_key_jwt,config_token_expire_sec,config_token_user_key_list)
    return {"status":1,"message":token}

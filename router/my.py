...
async def func_api_my_profile(request:Request):
   ...
   profile=await st.func_my_profile_read(st.client_postgres_pool, request.state.user["id"], st.config_sql)
   token=await st.func_token_encode(profile, st.config_token_secret_key, st.config_token_expiry_sec, st.config_token_refresh_expiry_sec, st.config_token_key)
   return {"status":1,"message":profile|token}

@router.post("/my/token-refresh")
async def func_api_my_token_refresh(request:Request):
   st=request.app.state
   user=await st.func_user_single_read(st.client_postgres_pool, request.state.user["id"])
...
async def func_api_my_object_create(request:Request):
   ...
   return {"status":1,"message":await st.func_orchestrator_obj_create("my",obj_query,obj_body,request.state.user.get("id"),st.config_table_create_my,st.config_table_create_public,st.config_column_blocked,st.client_postgres_pool,st.func_postgres_serialize,st.config_table,st.func_orchestrator_producer,st.client_celery_producer,st.client_kafka_producer,st.client_rabbitmq_producer,st.client_redis_producer,st.config_channel_allowed,st.func_postgres_object_create,st.config_postgres_batch_limit)}
...
async def func_api_my_object_update(request:Request):
   ...
   obj_body=await st.func_request_param_read(request,"body",[])
   return {"status":1,"message":await st.func_orchestrator_obj_update("my",obj_query,obj_body,request.state.user.get("id"),st.config_column_blocked,st.config_column_single_update,st.client_postgres_pool,st.func_postgres_serialize,st.func_orchestrator_producer,st.client_celery_producer,st.client_kafka_producer,st.client_rabbitmq_producer,st.client_redis_producer,st.config_channel_allowed,st.func_postgres_object_update,st.func_otp_verify,st.config_expiry_sec_otp,0,st.config_postgres_batch_limit)}
...
async def func_api_my_object_create_mongodb(request:Request):
   st=request.app.state
   obj_query=await st.func_request_param_read(request,"query",[("database","str",1,None,None,None,None),("table","str",1,None,None,None,None)])
   obj_body=await st.func_request_param_read(request,"body",[])
   obj_list=obj_body.get("obj_list", [obj_body])
   output=await st.func_mongodb_object_create(st.client_mongodb, obj_query["database"], obj_query["table"], obj_list)
   return {"status":1,"message":output}

...
async def func_api_my_api_usage(*, request: Request):
    app_state = request.app.state
    if not app_state.client_postgres_read_fallback: raise Exception("postgres read client not initialized")
    oq = await app_state.func_request_param_read(request=request, mode="query", strict=0, param_specs=[("days", "int", 1, None, None)])
...
async def func_api_my_object_create(*, request: Request):
    app_state = request.app.state
    oq = await app_state.func_request_param_read(request=request, mode="query", strict=0, param_specs=[("table", "str", 1, app_state.cache_postgres_schema_table_list, None), ("mode", "str", 0, ["now", "buffer"], "now"), ("queue", "str", 0, app_state.config_queue_services, None)])
...
async def func_api_my_object_read(*, request: Request):
    app_state = request.app.state
    if not app_state.client_postgres_read_fallback: raise Exception("postgres read client not initialized")
    oq = await app_state.func_request_param_read(request=request, mode="query", strict=0, param_specs=[("table", "str", 1, app_state.cache_postgres_schema_table_list, None), ("ownership_column", "str", 0, app_state.config_column_ownership, "created_by_id"), ("limit", "int", 0, None, app_state.config_sql_read_limit_default), ("page", "int", 0, None, 1), ("order", "str", 0, None, "id desc"), ("column", "str", 0, None, "*"), ("relation", "list", 0, None, []), ("filter", "list", 0, None, [])])
...
async def func_api_my_object_update(*, request: Request):
    app_state = request.app.state
    oq = await app_state.func_request_param_read(request=request, mode="query", strict=0, param_specs=[("table", "str", 1, app_state.cache_postgres_schema_table_list, None), ("otp", "int", 0, None, None), ("queue", "str", 0, app_state.config_queue_services, None)])
    ob = await app_state.func_request_param_read(request=request, mode="body", strict=0, param_specs=[])
    obj_list = ob.get("obj_list", [ob])
...
async def func_api_my_object_delete_all(*, request: Request):
    ...
    if not app_state.client_postgres: raise Exception("postgres client not initialized")
    oq = await app_state.func_request_param_read(request=request, mode="query", strict=0, param_specs=[("table", "str", 1, app_state.cache_postgres_schema_table_list, None)])
...
async def func_api_my_received_ids_delete(*, request: Request):
    app_state, user_id = request.app.state, request.state.user["id"]
    if not app_state.client_postgres: raise Exception("postgres client not initialized")
    ob = await app_state.func_request_param_read(request=request, mode="body", strict=0, param_specs=[("table", "str", 1, app_state.cache_postgres_schema_table_list, None), ("col", "str", 0, ["user_id"], "user_id"), ("ids", "list:int", 1, None, None)])
    if app_state.config_batch_item_limit and len(ob["ids"]) > app_state.config_batch_item_limit: raise Exception(f"maximum {app_state.config_batch_item_limit} objects allowed")
...
async def func_api_my_received_object_delete_all(*, request: Request):
    app_state, user_id = request.app.state, request.state.user["id"]
    if not app_state.client_postgres: raise Exception("postgres client not initialized")
    oq = await app_state.func_request_param_read(request=request, mode="query", strict=0, param_specs=[("table", "str", 1, app_state.cache_postgres_schema_table_list, None), ("col", "str", 0, ["user_id"], "user_id")])
...
async def func_api_my_message_inbox(*, request: Request):
    app_state = request.app.state
    if not app_state.client_postgres_read_fallback: raise Exception("postgres read client not initialized")
    oq = await app_state.func_request_param_read(request=request, mode="query", strict=0, param_specs=[("mode", "str", 1, ["all", "unread", "read"], None), ("order", "str", 0, None, "id desc"), ("limit", "int", 0, None, app_state.config_sql_read_limit_default), ("page", "int", 0, None, 1)])
...
async def func_api_my_message_thread(*, request: Request):
    app_state = request.app.state
    if not app_state.client_postgres: raise Exception("postgres client not initialized")
    oq = await app_state.func_request_param_read(request=request, mode="query", strict=0, param_specs=[("user_id", "int", 1, None, None), ("order", "str", 0, None, "id desc"), ("limit", "int", 0, None, app_state.config_sql_read_limit_default), ("page", "int", 0, None, 1)])
...
async def func_api_my_object_create_mongodb(*, request: Request):
    ...
    oq = await app_state.func_request_param_read(request=request, mode="query", strict=0, param_specs=[("database", "str", 1, None, None), ("table", "str", 1, None, None)])
    ob = await app_state.func_request_param_read(request=request, mode="body", strict=0, param_specs=[])
    obj_list = ob.get("obj_list", [ob])
...
async def func_api_my_blob_url_delete(*, request: Request):
    app_state = request.app.state
    ob = await app_state.func_request_param_read(request=request, mode="body", strict=0, param_specs=[("service", "str", 1, app_state.config_blob_services, None), ("url", "list:str", 1, None, None)])

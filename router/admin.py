...
async def func_api_admin_sync(*, request:Request):
   ...
   await st.func_postgres_object_create(client_postgres_pool=st.client_postgres_pool, func_postgres_serialize=st.func_postgres_serialize, cache_postgres_schema=st.cache_postgres_schema, execution_mode="flush", table_name="", obj_list=[], is_serialize=0, table_buffer_limit=0, cache_postgres_buffer=st.cache_postgres_buffer)
...
async def func_api_admin_postgres_runner(*, request:Request):
   st=request.app.state
   obj_body=await st.func_request_param_read(request_obj=request, parsing_mode="body", param_config=[("mode","str",1,["read","write"],None,None,None),("query","str",1,None,None,None,None)], is_strict=0)
...
async def func_api_admin_postgres_export(*, request:Request):
   st=request.app.state
   obj_body=await st.func_request_param_read(request_obj=request, parsing_mode="body", param_config=[("query","str",1,None,None,None,None)], is_strict=0)
...
async def func_api_admin_postgres_import(*, request:Request):
   ...
   obj_form=await st.func_request_param_read(request_obj=request, parsing_mode="form", param_config=[("mode","str",1,["create","update","delete"],None,None,None),("table","str",1,st.cache_postgres_schema_tables,None,None,None),("file","file",1,[],None,None,None)], is_strict=0)
   async for obj_list in st.func_api_file_to_chunks(upload_file=obj_form["file"][-1], chunk_size=st.config_limit_obj_list):
      if obj_form["mode"]=="create":
         await st.func_postgres_object_create(client_postgres_pool=st.client_postgres_pool, func_postgres_serialize=st.func_postgres_serialize, cache_postgres_schema=st.cache_postgres_schema, table_name=obj_form["table"], obj_list=obj_list, table_buffer_limit=0, cache_postgres_buffer=st.cache_postgres_buffer)
      elif obj_form["mode"]=="update":
         await st.func_postgres_object_update(client_postgres_pool=st.client_postgres_pool, func_postgres_serialize=st.func_postgres_serialize, cache_postgres_schema=st.cache_postgres_schema, table_name=obj_form["table"], obj_list=obj_list)
      elif obj_form["mode"]=="delete":
         await st.func_ids_delete(client_postgres_pool=st.client_postgres_pool, table=obj_form["table"], ids=",".join(str(obj["id"]) for obj in obj_list), created_by_id=0, config_table_system=st.config_table_system, config_postgres_ids_delete_limit=st.config_postgres_ids_delete_limit)
      count += len(obj_list)
...
async def func_api_admin_object_update(*, request:Request):
   st=request.app.state
   obj_query=await st.func_request_param_read(request_obj=request, parsing_mode="query", param_config=[("table","str",1,st.cache_postgres_schema_tables,None,None,None),("is_serialize","int",0,[0,1],0,None,None),("otp","int",0,None,None,None,None),("queue","str",0,None,None,None,None)], is_strict=0)
   obj_body=await st.func_request_param_read(request_obj=request, parsing_mode="body", param_config=[], is_strict=0)
...
async def func_api_admin_ids_delete(*, request:Request):
   st=request.app.state
   obj_body=await st.func_request_param_read(request_obj=request, parsing_mode="body", param_config=[("table","str",1,st.cache_postgres_schema_tables,None,None,None),("ids","str",1,None,None,None,None)], is_strict=0)
   output=await st.func_ids_delete(client_postgres_pool=st.client_postgres_pool, table=obj_body["table"], ids=obj_body["ids"], created_by_id=0, config_table_system=st.config_table_system, config_postgres_ids_delete_limit=st.config_postgres_ids_delete_limit)
   return {"status":1,"message":output}

@router.post("/admin/redis-import-create")
async def func_api_admin_redis_import_create(*, request:Request):
   st, count = request.app.state, 0
   obj_form=await st.func_request_param_read(request_obj=request, parsing_mode="form", param_config=[("table","str",1,st.cache_postgres_schema_tables,None,None,None),("file","file",1,[],None,None,None),("expiry_sec","int",0,None,None,None,None)], is_strict=0)
   async for obj_list in st.func_api_file_to_chunks(upload_file=obj_form["file"][-1], chunk_size=st.config_limit_obj_list):
...
async def func_api_admin_redis_import_delete(*, request:Request):
   st, count = request.app.state, 0
   obj_form=await st.func_request_param_read(request_obj=request, parsing_mode="form", param_config=[("table","str",1,st.cache_postgres_schema_tables,None,None,None),("file","file",1,[],None,None,None)], is_strict=0)
   async for obj_list in st.func_api_file_to_chunks(upload_file=obj_form["file"][-1], chunk_size=st.config_limit_obj_list):
      key_list=[f"""{obj_form["table"]}_{item["id"]}""" for item in obj_list]
...
async def func_api_admin_mongodb_import(*, request:Request):
   st, count = request.app.state, 0
   obj_form=await st.func_request_param_read(request_obj=request, parsing_mode="form", param_config=[("mode","str",1,["create","delete"],None,None,None),("database","str",1,None,None,None,None),("table","str",1,st.cache_postgres_schema_tables,None,None,None),("file","file",1,[],None,None,None)], is_strict=0)
   async for obj_list in st.func_api_file_to_chunks(upload_file=obj_form["file"][-1], chunk_size=st.config_limit_obj_list):
      if obj_form["mode"]=="create":
...
async def func_api_admin_s3_bucket_ops(*, request:Request):
   st=request.app.state
   obj_query=await st.func_request_param_read(request_obj=request, parsing_mode="query", param_config=[("mode","str",1,["create","public","empty","delete"],None,None,None),("bucket","str",1,None,None,None,None)], is_strict=0)
   if obj_query["mode"]=="create":
      output=await st.func_s3_bucket_create(client_s3=st.client_s3, config_s3_region_name=st.config_s3_region_name, bucket_name=obj_query["bucket"])
   elif obj_query["mode"]=="public":
      output=await st.func_s3_bucket_public(client_s3=st.client_s3, bucket_name=obj_query["bucket"])
   elif obj_query["mode"]=="empty":
      output=st.func_s3_bucket_empty(client_s3_resource=st.client_s3_resource, bucket_name=obj_query["bucket"])
   elif obj_query["mode"]=="delete":
      output=await st.func_s3_bucket_delete(client_s3=st.client_s3, bucket_name=obj_query["bucket"])
   return {"status":1,"message":output}

@router.post("/admin/s3-url-delete")
async def func_api_admin_s3_url_delete(*, request:Request):
   st=request.app.state
   obj_body=await st.func_request_param_read(request_obj=request, parsing_mode="body", param_config=[("url","list",1,[],None,None,None)], is_strict=0)
   output=st.func_s3_url_delete(client_s3_resource=st.client_s3_resource, url_list=obj_body["url"])
   return {"status":1,"message":output}

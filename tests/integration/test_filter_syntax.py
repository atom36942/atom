...
async def test_flat_list_filtering(integration_app):
    ...
    await pool.execute("INSERT INTO test (title, type, views, rating) VALUES ('Apple iPhone', 1, 1, 4.5)")
    await pool.execute("INSERT INTO test (title, type, views, rating) VALUES ('Samsung Galaxy', 1, 1, 4.8)")
    await pool.execute("INSERT INTO test (title, type, views, rating) VALUES ('Nokia 3310', 2, 0, 3.0)")
    await pool.execute("INSERT INTO test (title, type, views, rating) VALUES ('Google Pixel', 1, 1, 4.2)")

    # 2. Test simple equality in list
    payload = {
        "table": "test",
        "filter": ["type = 1", "views = 1"]
    }
    res = await integration_app.get("/public/object-read", params={"table": "test", "filter": json.dumps(["type = 1", "views = 1"])})

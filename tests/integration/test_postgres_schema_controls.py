    ...
    rows = await conn.fetch(
        ...
        FROM (
          VALUES
            ...
            ('is_enable_drop_schema_or_table_disable', EXISTS (SELECT 1 FROM pg_event_trigger WHERE evtname = 'trigger_drop_disable')),
            ('is_enable_drop_column_disable', EXISTS (SELECT 1 FROM pg_event_trigger WHERE evtname = 'trigger_drop_column_disable')),
            ('is_enable_truncate_disable_users', EXISTS (SELECT 1 FROM pg_trigger t JOIN pg_class c ON c.oid = t.tgrelid WHERE c.relname = 'users' AND t.tgname = 'trigger_truncate_disable_users' AND NOT t.tgisinternal)),
            ('is_enable_users_delete_role_disable', EXISTS (SELECT 1 FROM pg_trigger t JOIN pg_class c ON c.oid = t.tgrelid JOIN pg_proc p ON p.oid = t.tgfoid WHERE c.relname = 'users' AND t.tgname = 'trigger_delete_disable_role_users' AND p.proname = 'func_delete_disable_role_users' AND NOT t.tgisinternal)),
    ...
    with PostgresContainer("postgis/postgis:16-3.4-alpine") as postgres:
        ...
        try:
            ...
            assert checks == {
                ...
                "is_enable_drop_schema_or_table_disable": False,
                "is_enable_drop_column_disable": False,
                "is_enable_truncate_disable_users": False,
                "is_enable_users_delete_role_disable": True,
                ...
                config_postgres=minimal_control_config(
                    {
                        ...
                        "is_enable_users_delete_role_disable": 0,

...
async def test_error_handler_unique_violation():
    ...
    msg, resp = await app.state.func_middleware_api_response_error(exception=exc, is_traceback=0, sentry_dsn=None)
...
async def test_error_handler_not_null_violation():
    ...
    # In app.state.func_middleware_api_response_error it looks for "message"
...
async def test_error_handler_sentry_capture():
    ...
    with patch("sentry_sdk.capture_exception") as mock_capture:
        ...
        with patch.dict("sys.modules", {"sentry_sdk": MagicMock()}):
            import sentry_sdk
            await app.state.func_middleware_api_response_error(exception=exc, is_traceback=0, sentry_dsn="http://dsn")

...
# logic
async def execute():
    sensitive_tables = ["users", "spatial_ref_sys"]
    blocked_tables = [table for table, cfg in config_table.items() if cfg.get("retention_day") is not None and table in sensitive_tables]
    if blocked_tables:

...
def parse_args():
    ...
    parser.add_argument("--password", default=DEFAULT_PASSWORD, help="Default password for newly-created buyer users.")
    parser.add_argument("--user-type", type=int, default=DEFAULT_USER_TYPE, help="users.type value for buyer users.")
    parser.add_argument("--role", type=int, default=DEFAULT_USER_ROLE, help="users.role value for buyer users.")
...
def buyer_sql(*, include_inactive: bool) -> str:
    ...
    return f"""
        WITH BuyerIds AS (
            ...
            SELECT DISTINCT OL_OH_Buyer AS BuyerPK
...
async def fetch_cargowise_buyers(*, mssql_url: str, include_inactive: bool) -> list[dict]:
    pool = await aioodbc.create_pool(dsn=mssql_url, minsize=1, maxsize=3)
    try:
        async with pool.acquire() as conn:
            cursor = await conn.cursor()
            await cursor.execute(buyer_sql(include_inactive=include_inactive))
...
async def read_existing_users(*, client_postgres_pool, buyers: list[dict], user_type: int) -> list[dict]:
    if not buyers:
        return []
    id_exts = [buyer["id_ext"] for buyer in buyers]
    usernames = [buyer["username"] for buyer in buyers]
    sql = """
        SELECT id, type, username, id_ext, name, role
        FROM users
        WHERE type = $1
          AND (
              id_ext = ANY($2::text[])
...
def plan_user_changes(*, buyers: list[dict], existing_users: list[dict], password: str, user_type: int, role: int) -> tuple[list[dict], list[dict], list[str]]:
    existing_by_id_ext = {}
...
async def create_users(*, client_postgres_pool, cache_postgres_schema: dict, client_password_hasher, obj_list: list[dict]) -> int:
    total = 0
    cache_postgres_buffer_create = {}
    batch_size = min(BATCH_SIZE, config_obj_list_limit or BATCH_SIZE)
    for batch in chunked(obj_list, batch_size):
        await func_postgres_create(
            ...
            cache_postgres_buffer_create=cache_postgres_buffer_create,
            config_regex=config_regex,
            config_table=config_table,
            config_obj_list_limit=config_obj_list_limit,
            buffer_limit=config_table.get("users", {}).get("buffer_limit", config_buffer_limit_default),
            mode="now",
            table="users",
            obj_list=batch,
        )
...
async def update_users(*, client_postgres_pool, cache_postgres_schema: dict, client_password_hasher, obj_list: list[dict]) -> int:
    total = 0
    batch_size = min(BATCH_SIZE, config_obj_list_limit or BATCH_SIZE)
    for batch in chunked(obj_list, batch_size):
...
async def execute():
    ...
    buyers = await fetch_cargowise_buyers(mssql_url=args.mssql_url, include_inactive=args.include_inactive)
    ...
    try:
        ...
        existing_users = await read_existing_users(client_postgres_pool=client_postgres_pool, buyers=buyers, user_type=args.user_type)
        create_list, update_list, duplicate_warnings = plan_user_changes(buyers=buyers, existing_users=existing_users, password=args.password, user_type=args.user_type, role=args.role)
        ...
        created_count = await create_users(client_postgres_pool=client_postgres_pool, cache_postgres_schema=cache_postgres_schema, client_password_hasher=client_password_hasher, obj_list=create_list) if create_list else 0
        updated_count = await update_users(client_postgres_pool=client_postgres_pool, cache_postgres_schema=cache_postgres_schema, client_password_hasher=client_password_hasher, obj_list=update_list) if update_list else 0

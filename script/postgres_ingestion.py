...
TABLE = "users"
CRUD_MODE = "create"  # Options: 'create', 'update', 'delete'
VALIDATION_MODE = "strict"  # Options: 'strict', 'reject', 'loose'
RENAME_COLUMN = [["name_old", "name"]]
IGNORE_COLUMN = ["ignored_col"]
CONST_COLUMN = [["is_active", "true"]]
...
if __name__ == "__main__":
    ...
    async def main():
        print("Starting sample CSV ingestion...")
        if not PG_URL:
            raise ValueError("PG_URL environment variable is not set. Please set it before running this script.")
        
        # Create a mock CSV file for demonstration if it doesn't exist
        if not os.path.exists(CSV_PATH):
            with open(CSV_PATH, "w", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(["id", "name_old", "email", "ignored_col"])
                writer.writerow(["1", "Alice", "alice@example.com", "skip_me"])
                writer.writerow(["2", "Bob", "bob@example.com", "skip_me"])
            print(f"Created a mock CSV file: {CSV_PATH}")

        try:
            result = await func_postgres_csv_ingestion(
                csv_path=CSV_PATH,
                pg_dsn=PG_URL,
                table=TABLE,
                crud_mode=CRUD_MODE,

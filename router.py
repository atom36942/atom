...
async def func_api_770160e847a341998b5b1c698e52e5c4(request:Request):
   ...
   obj_body=(await func_postgres_obj_serialize(request.app.state.cache_postgres_column_datatype,[obj_body]))[0]
   ...
   token=await func_jwt_token_encode(user,config_key_jwt,config_token_expiry_sec,config_token_key_list)
...
async def func_api_27860b1e950446a9b5bd28c2e9a33de4(request:Request):
   ...
   response.headers["Authorization"]=f"{await func_jwt_token_encode(user,config_key_jwt,config_token_expiry_sec,config_token_key_list)}"
   return response

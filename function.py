...
def func_postgres_schema_config_validate(*, config_db: dict) -> None:
    ...
    for table_name, column_configs in config_db["table"].items():
        ...
        allowed_primary_datatypes = {"bigserial", "bigint generated always as identity", "bigint generated by default as identity"}
        if primary_cfg.get("name") != "id" or primary_cfg.get("is_primary") != 1 or primary_cfg.get("datatype") not in allowed_primary_datatypes:
            raise Exception(f"{table_name} first column must have name='id', is_primary=1, and datatype in {allowed_primary_datatypes}")

...
async def gsheet_read_service_account(client_gsheet,spreadsheet_id,sheet_name,cell_boundary):
...
async def openai_prompt(client_openai,model,prompt,is_web_search,previous_response_id):
...
import base64
async def openai_ocr(client_openai,model,prompt,file):
...
async def function_postgres_create(table,object_list,is_serialize,client_postgres,postgres_column_datatype,function_object_serialize):
...
async def function_postgres_update(table,object_list,is_serialize,client_postgres,postgres_column_datatype,function_object_serialize):
   if is_serialize:object_list=await function_object_serialize(object_list,postgres_column_datatype)
...
async def function_postgres_update_user(table,object_list,is_serialize,client_postgres,postgres_column_datatype,function_object_serialize,user_id):
   if is_serialize:object_list=await function_object_serialize(object_list,postgres_column_datatype)
   column_update_list=[*object_list[0]]
   column_update_list.remove("id")
...
async def function_postgres_delete(table,object_list,is_serialize,client_postgres,postgres_column_datatype,function_object_serialize):
   if is_serialize:object_list=await function_object_serialize(object_list,postgres_column_datatype)
...
async def function_postgres_delete_any(table,object,client_postgres,create_where_string,function_object_serialize,postgres_column_datatype):
...
async def function_postgres_read(table,object,client_postgres,postgres_column_datatype,function_object_serialize,create_where_string):
...
async def function_postgres_parent_read(table,parent_column,parent_table,client_postgres,order,limit,offset,user_id):
   query=f'''
   with
...
import random
async def function_generate_save_otp(client_postgres,email,mobile):
...
async def function_verify_otp(client_postgres,otp,email,mobile):
   if email and mobile:raise Exception("send either email/mobile")
   if not email and not mobile:raise Exception("send either email/mobile")
   if email:
...
async def function_add_creator_data(client_postgres,object_list,user_key):
...
async def postgres_query_runner(client_postgres,query,user_id):
...
async def function_postgres_update_ids(client_postgres,table,ids,column,value,updated_by_id,created_by_id):
...
async def function_postgres_delete_ids(client_postgres,table,ids,created_by_id):
...
import json
async def redis_set_object(client_redis,key,expiry,object):
...
import json
async def redis_get_object(client_redis,key):
...
async def function_delete_user(mode,client_postgres,user_id):
...
async def users_api_access_read(client_postgres_asyncpg,limit):
   users_api_access={}
...
import json
object_list_log_api=[]
async def log_api_create(object,batch,function_postgres_create,client_postgres,postgres_column_datatype,function_object_serialize):
   try:
      global object_list_log_api
      object_list_log_api.append(object)
      if len(object_list_log_api)>=batch:
         await function_postgres_create("log_api",object_list_log_api,0,client_postgres,postgres_column_datatype,function_object_serialize)
         object_list_log_api=[]
   except Exception as e:print(str(e))
   return None
...
async def function_mark_message_object_read(client_postgres,object_list):
   try:
...
import datetime
async def function_update_user_last_active_at(client_postgres,user_id):
   try:
...
async def mongodb_create_object(client_mongodb,database,table,object_list):
   mongodb_client_database=client_mongodb[database]
   output=await mongodb_client_database[table].insert_many(object_list)
   return str(output)

async def function_message_inbox_user(client_postgres,user_id,order,limit,offset,is_unread):
...
async def function_message_received_user(client_postgres,user_id,order,limit,offset,is_unread):
...
async def function_message_thread_user(client_postgres,user_id_1,user_id_2,order,limit,offset):
...
async def function_mark_message_read_thread(client_postgres,user_id_1,user_id_2):
...
async def function_sns_send_message_template(client_sns,mobile,template_id,entity_id,sender_id,message):
   client_sns.publish(
      PhoneNumber=mobile,
      Message=message,
...
async def function_sns_send_message(client_sns,mobile,message):
...
import hashlib
async def function_signup_username_password(client_postgres,type,username,password):
...
async def function_signup_username_password_bigint(client_postgres,type,username_bigint,password_bigint):

...
async def function_api_response_cache(mode,request,response,redis_client):
   ...
   if mode=="set":
      ...
      await redis_client.setex(cache_key,100,base64.b64encode(gzip.compress(body)).decode())
      response=Response(content=body, status_code=response.status_code, media_type=response.media_type)
   return response

async def function_rate_limiter_check(request,api_config,redis_client):
...
async def function_api_access_check(request,api_config,users_api_access,postgres_client):
...
async def function_is_active_check(request,users_is_active,postgres_client):

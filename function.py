...
async def func_mgh_amazon_invoice_logic(request):
    ...
    CERT_FILES = ("secret/mgh_amazon_cert.txt", "secret/mgh_amazon_key.key")
    SFTP_PATH = "mgh/amazon"
    EXPORT_PREFIX = "export/mgh_amazon_"
    TABLE_NAME = "mgh_amazon_invoice"
    AMAZON_CLIENT_ID = request.app.state.config_amazon_client_id
    AMAZON_CLIENT_SECRET = request.app.state.config_amazon_client_secret
    state = request.app.state
    if not state.client_sftp: raise Exception("sftp client not connected")
    func = func_mgh_amazon_invoice_logic
    if not hasattr(func, "_token"): func._token = func._expires = None
    files = await state.func_sftp_folder_filename_read(state.client_sftp, SFTP_PATH)
    process_results = []
    for filename in [f for f in files if f not in (".", "..") and f.lower().endswith(".csv")]:
        local_path = f"{EXPORT_PREFIX}{filename}"
        await state.func_sftp_file_download(state.client_sftp, f"{SFTP_PATH}/{filename}", local_path)
        db_records = []
        for row in state.func_csv_to_obj_list(local_path):
            ...
            payload = {
                "invoiceDocument": {
                    "invoiceId": invoice_id, "accountId": row["AccountID"], "invoiceDocumentOwner": {"supplierId": "MGHP", "supplierIdType": "SCAC"},
                    "invoiceHeader": {
                        "invoiceNumber": invoice_id, "purchaseOrderNumber": invoice_id, "invoiceDate": row["INVOICEDATE"],
                    ...
                    "invoiceLineItems": [{
                        "lineItemId": "1", "description": row["DESCRIPTION"], "lineItemRequisitionIdentifierInfo": {"primaryRefId": {"extensions": [{"trackingId": row["TRACKINGID"]}]}},
                        "lineItemDetail": {
                            "freightShipmentDetail": {
                                "shipmentIdentificationNumber": row["FBANumber"], "shipmentBillOfLadingNumber": row["SHIPMENTBILLOFLADINGNUMBER"], "proofOfDelivery": row["PROOFOFDELIVERY"],
                                ...
                                "transportationMethodType": row["TRANSPORTATIONMETHODTYPE"], "shippedDate": row["SHIPPEDDATE"], "deliveryDate": row["DELIVERYDATE"],
                                "shipmentWeightDetail": {"grossWeight": {"weight": float(row["GROSSWEIGHT"]), "unitOfMeasurement": "KG"}, "netWeight": {"weight": float(row["GROSSWEIGHT"]), "unitOfMeasurement": "KG"}, "billedWeight": {"weight": float(row["GROSSWEIGHT"]), "unitOfMeasurement": "KG"}, "declaredWeight": {"weight": float(row["GROSSWEIGHT"]), "unitOfMeasurement": "KG"}},
                                "shipmentVolumeDetail": {"billedVolume": {"length": float(row["VOLUMELENGTH"]), "breadth": float(row["VOLUMEBREADTH"]), "height": float(row["VOLUMEHEIGHT"]), "unitOfMeasurement": "GALLONS"}, "declaredVolume": {"length": float(row["VOLUMELENGTH"]), "breadth": float(row["VOLUMEBREADTH"]), "height": float(row["VOLUMEHEIGHT"]), "unitOfMeasurement": "GALLONS"}},
                                "packageDetails": [{
                                    "packageReferenceId": row["CHARGEID"],
                                    "packageWeightDetail": {"grossWeight": {"weight": float(row["GROSSWEIGHT"]), "unitOfMeasurement": "KG"}, "netWeight": {"weight": float(row["GROSSWEIGHT"]), "unitOfMeasurement": "KG"}, "billedWeight": {"weight": float(row["GROSSWEIGHT"]), "unitOfMeasurement": "KG"}, "declaredWeight": {"weight": float(row["GROSSWEIGHT"]), "unitOfMeasurement": "KG"}},
                                    "packageVolumeDetail": {"billedVolume": {"length": float(row["VOLUMELENGTH"]), "breadth": float(row["VOLUMEBREADTH"]), "height": float(row["VOLUMEHEIGHT"]), "unitOfMeasurement": "GALLONS"}, "declaredVolume": {"length": float(row["VOLUMELENGTH"]), "breadth": float(row["VOLUMEBREADTH"]), "height": float(row["VOLUMEHEIGHT"]), "unitOfMeasurement": "GALLONS"}}
                                }]
                            }
                        },
                        "unitCharges": [{"chargeId": row["CHARGEID"], "chargeCode": row["CHARGECODE"], "chargeAmount": {"amount": float(row["AMOUNT"]), "currencyCode": row["CURRENCYCODE"]}}],
            ...
            if not func._token or not func._expires or now_utc >= func._expires:
                r_tk = await state.client_http.post(TOKEN_URL, data={"grant_type": "client_credentials", "client_id": AMAZON_CLIENT_ID, "client_secret": AMAZON_CLIENT_SECRET}, headers={"Content-Type": "application/x-www-form-urlencoded"})
                r_tk.raise_for_status()
                func._token, func._expires = r_tk.json()["access_token"], now_utc + timedelta(minutes=55)
            r = await state.client_http.post(API_URL, json=payload, headers={"Authorization": f"Bearer {func._token}", "Content-Type": "application/json"}, cert=CERT_FILES, timeout=60)
            ok = r.status_code in (200, 202)
            db_records.append({"invoice_id": invoice_id, "payload": payload, "filename": filename, "status": 1 if ok else 0, "response": (r.json() if r.content else None) if ok else r.text})
        if db_records: process_results.append(await state.func_postgres_obj_create(state.client_postgres_pool, state.func_postgres_obj_serialize, state.cache_postgres_column_datatype, "now", TABLE_NAME, db_records, 1))

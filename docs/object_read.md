...
| :--- | :--- | :--- |
| **`GET /public/object-read?table=<tbl>`** | Public | Read table records with public access filters (`config_table_public_read_allowed`) |
| **`GET /private/object-read?table=<tbl>`** | Authenticated User | Read table records with private access filters (`config_table_private_read_allowed`) |
| **`GET /my/object-read?table=<tbl>`** | Authenticated User | Scoped strictly to user-created records (`created_by_id = current_user.id`) |
| **`GET /my/object-read-owned?table=<tbl>&ownership_column=<col>`** | Authenticated User | Scoped to custom ownership column (`<ownership_column> = current_user.id`) with auto-mark-as-read |

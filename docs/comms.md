...
The chosen provider's client/keys must be configured (see [setup.md](setup.md)) or the call errors. Logic lives in `func_email_send`, `func_otp_send_email`, and `func_otp_send_mobile` in [`function.py`](../function.py).

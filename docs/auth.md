...
1. **Send** — `POST /public/otp-send-email` or `/public/otp-send-mobile` → `func_otp_generate` creates a `config_otp_length`-digit code, stores it in the `otp` table, and sends it via the chosen email/SMS service (see [email.md](email.md) and [sms.md](sms.md)).
